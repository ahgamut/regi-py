"""Multiprocessing orchestration for the AlphaDouZero (ADZ) pipeline.

A near-copy of ``trainers/az_trainer.py`` -- same three process roles (trainer /
explorer / evaluator) + ``submain``, same auto ``--num-processes`` sizing, same
``eval_done`` shutdown handshake -- swapping AZ self-play/eval for the ADZ
candidate-scoring versions and iterating over the SEPARATE ADZ net registry
(``get_adz_net`` / ``adz_net_names``), so ``adz_trainer --net`` can never
instantiate a card-space ``BaseNet``. ``run_epoch`` / ``get_split_optimizer`` /
``drain`` / ``ShardBuffer`` are reused unchanged (the shared, paradigm-agnostic
plumbing).
"""
import argparse
import os
    ...
    try:
        while True:
            ...
            if not have_baseline:
                ...
                have_baseline = True
                adz_test_model(episode, new_model, params.num_simulations)
                continue
            # otherwise only checkpoint when the candidate beats the last-saved one
            if adz_improved_gameplay(
                episode,
                new_model=new_model,
                old_model=old_model,
                num_simulations=10,
...
def main():
    parser = argparse.ArgumentParser("regi-adz-trainer")
    parser.add_argument(
